...
#define QINT64_MIN        (-9223372036854775807i64 - 1)
#define QINT64_MAX        9223372036854775807i64
...
namespace xsd
{
	...
	{
		static const qint64 value = 10 * DecimalFactor<Prec - 1>::value;
	};

	template <>
	struct DecimalFactor<0>
	{
		static const qint64 value = 1;
	};

	template <>
	struct DecimalFactor<1>
	{
		static const qint64 value = 10;
	};

	template <int Prec, bool positive>
	struct DecimalFactorDiff_impl
	{
		static const qint64 value = DecimalFactor<Prec>::value;
	};

	template <int Prec>
	struct DecimalFactorDiff_impl<Prec, false>
	{
		static const qint64 value = QINT64_MIN;
	};

	template <int Prec>
	struct DecimalFactorDiff
	{
		static const qint64 value = DecimalFactorDiff_impl<Prec, Prec >= 0>::value;
	};

	// round floating point value and convert to int64
	template <class T>
	inline qint64 round(T value)
	{
		T val1;
		...
		qint64 intPart = static_cast<qint64>(val1);

		return intPart;
	}

	// calculate output = round(a / b), where output, a, b are int64
	inline bool div_rounded(qint64& output, qint64 a, qint64 b)
	{
		qint64 divisorCorr = std::abs(b) / 2;
		if (a >= 0) {
			if (QINT64_MAX - a >= divisorCorr) {
				output = (a + divisorCorr) / b;
				return true;
			}
		}
		else {
			if (-(QINT64_MIN - a) >= divisorCorr) {
				output = (a - divisorCorr) / b;
				return true;
			}
		}
	...
	public:
		...
		inline static qint64 multDiv(const qint64 value1, const qint64 value2,
		                            qint64 divisor)
		{
			// we don't check for division by zero, the caller should - the next line will throw.
			const qint64 value1int = value1 / divisor;
			qint64 value1dec = value1 % divisor;
			const qint64 value2int = value2 / divisor;
			qint64 value2dec = value2 % divisor;

			qint64 result = value1 * value2int + value1int * value2dec;
			...
			if (!isMultOverflow(value1dec, value2dec)) {
				// no overflow
				qint64 resDecPart = value1dec * value2dec;
			...
			{
				qint64 c = gcd(value1dec, divisor);
				if (c != 1) {
					value1dec /= c;
					divisor /= c;
				}
		...
		static bool isMultOverflow(const qint64 value1, const qint64 value2)
		{
			...
			if ((value1 < 0) != (value2 < 0)) {
				// different sign
				if (value1 == QINT64_MIN) {
					return value2 > 1;
				}
				else if (value2 == QINT64_MIN) {
					return value1 > 1;
				}
				if (value1 < 0) {
				...
				if (value2 == QINT64_MIN) {
					return value1 < -1;
				}
				return isMultOverflow(-value1, -value2);
			}

			return (value1 > QINT64_MAX / value2);
		}

		static qint64 pow10(int n)
		{
			static const qint64 decimalFactorTable[] = {
				1, 10, 100, 1000, 10000,
	...
	public:
		template <class T>
		static qint64 round(T value)
		{
			return static_cast<qint64>(value);
		}

		static bool div_rounded(qint64& output, qint64 a, qint64 b)
		{
			output = a / b;
			return true;
		}
	...
	public:
		...
		static qint64 round(T value)
		{
			return xsd::round(value);
		}

		static bool div_rounded(qint64& output, qint64 a, qint64 b)
		{
			return xsd::div_rounded(output, a, b);
		}
	};
	...
	public:
		...
		{
			qint64 divisorCorr = std::abs(b) / 2;
			qint64 remainder = std::abs(a) % std::abs(b);
		...
		static qint64 round(T value)
		{
			...
			if (value >= 0.0) {
				...
				if (decimals > 0.5) {
					...
					bool is_even = (static_cast<qint64>(value - decimals) % 2 == 0);
					if (is_even) {
						val1 = floor(value);
					}
					else {
			...
			return static_cast<qint64>(val1);
		}

		static bool div_rounded(qint64& output, qint64 a, qint64 b)
		{
			qint64 divisorDiv2 = std::abs(b) / 2;
			qint64 remainder = std::abs(a) % std::abs(b);
	...
	public:
		...
		static qint64 round(T value)
		{
			return static_cast<qint64>(ceil(value));
		}
	...
	public:
		...
		static qint64 round(T value)
		{
			return static_cast<qint64>(floor(value));
		}
	...
	public:
		...
		explicit decimal(qint64 value, qint64 precFactor)
		{
		...
		const decimal operator*(qint64 rhs) const
		{
			decimal result = *this;
			result.m_value *= rhs;
			return result;
		...
		qint64 getUnbiased() const
		{
			return m_value;
		}

		void setUnbiased(qint64 value)
		{
			m_value = value;
		}
		...
		void setAsInteger(qint64 value)
		{
		...
		void unpack(qint64& beforeValue, qint64& afterValue) const
		{
		...
		/// \result Returns *this
		decimal& pack(qint64 beforeValue, qint64 afterValue)
		{
			if (Prec > 0) {
		...
		decimal& pack_rounded(qint64 beforeValue, qint64 afterValue)
		{
		...
		static decimal buildWithExponent(qint64 mantissa, int exponent)
		{
			decimal result;
		...
		static decimal& buildWithExponent(decimal& output, qint64 mantissa,
		                                  int exponent)
		{
		...
		void setWithExponent(qint64 mantissa, int exponent)
		{
		...
		void getWithExponent(qint64& mantissa, int& exponent) const
		{
	...
	protected:
		...
		void initWithPrec(qint64 value, qint64 precFactor)
		{
			qint64 ownFactor = DecimalFactor<Prec>::value;
	...
	template <int Prec>
	decimal<Prec> decimal_cast(qint64 arg)
	{
		decimal<Prec> result(arg);
		return result;
	}
	...
	template <int Prec, typename RoundPolicy>
	decimal<Prec, RoundPolicy> decimal_cast(qint64 arg)
	{
		decimal<Prec, RoundPolicy> result(arg);
		return result;
	}
	...
	namespace details
	{
		...
		bool parse_unpacked(StreamType& input, int& sign, qint64& before, qint64& after,
		                    int& decimalDigits)
		{
			using namespace std;
