...
namespace Ods {
    ...
    const QString CellFloat::kCellTypeValue = "float";
    const QString CellFloat::kCellTypeAttribute = "office:value";
    ...
    {
        writer->writeAttribute(kCellTypeAttribute, QString::number(_valueNumber, 'f', 2));
    }

    void CellFloat::SerializeSubitems(QXmlStreamWriter* writer)
    {
        writer->writeTextElement(CellAbstract::kTextTag, QString::number(_valueNumber, 'f', 2).replace(".", ","));
    }
}